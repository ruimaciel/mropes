...
mreturn_t mrope_prepend_rope(mrope_t *rope_inout, mrope_t *rope_inout_lhs) {
  ...
  new_root =
      mrope_make_branch_node(rope_inout_lhs->root_node, rope_inout->root_node);
  if (new_root == NULL) {
...
mreturn_t mrope_append_text(mrope_t *rope_inout, char *text_rhs) {
  ...
  if ((error = mrope_make_leaf_node_from_text(&leaf_node, text_rhs)) !=
      MROPE_OK) {
    return error;
  }
  ...
  switch (rope_inout->root_node->type) {
    case MROPE_NODE_LEAF: {
      ...
      if ((branch_node = mrope_make_branch_node(
               rope_inout->root_node, (struct mrope_node *)leaf_node)) ==
          NULL) {
    ...
    case MROPE_NODE_BRANCH: {
      struct mrope_branch_node *branch_root_node =
          (struct mrope_branch_node *)rope_inout->root_node;
      ...
      if (branch_root_node->right == NULL) {
        branch_root_node->right = (struct mrope_node *)leaf_node;
        branch_root_node->node.weight =
            branch_root_node->left->weight + branch_root_node->right->weight;
        return MROPE_OK;
      }

      struct mrope_branch_node *branch = NULL;
      if ((branch = mrope_make_branch_node(
               (struct mrope_node *)rope_inout->root_node,
               (struct mrope_node *)leaf_node)) == NULL) {
        mrope_free_leaf_node(leaf_node);
        return MROPE_MALLOC_FAILED;
      }
...
mreturn_t mrope_append_rope(mrope_t *rope_inout, mrope_t *rope_inout_rhs) {
  ...
  new_root =
      mrope_make_branch_node(rope_inout->root_node, rope_inout_rhs->root_node);
  if (new_root == NULL) {
    return MROPE_MALLOC_FAILED;
  }
...
mreturn_t mrope_concat(const mrope_t *rope_in_lhs, const mrope_t *rope_in_rhs,
                       mrope_t *rope_out) {
  mrope_t cloned_lhs;
  mrope_t cloned_rhs;
  struct mrope_branch_node *branch_node = NULL;
  ...
  branch_node =
      mrope_make_branch_node(cloned_lhs.root_node, cloned_rhs.root_node);
...
mreturn_t mrope_split(const mrope_t *rope_in, const size_t index,
                      mrope_t *rope_out_lhs, mrope_t *rope_out_rhs) {
  mreturn_t error = MROPE_OK;
  ...
  while (mrope_stack_top(&node_stack) != NULL) {
    ...
    switch (node->type) {
      case MROPE_NODE_BRANCH: {
        struct mrope_branch_node *branch_node =
            (struct mrope_branch_node *)(node);
      ...
      case MROPE_NODE_LEAF: {
        ...
        if (index > current_index + leaf_node->node.weight) {
          ...
          if ((lhs_string = realloc(duplicate_string, offset * sizeof(char))) ==
              NULL) {
...
mreturn_t mrope_insert(mrope_t *rope_inout, const size_t index,
                       mrope_t *rope_in) {
  mreturn_t error = MROPE_OK;
...
mreturn_t mrope_clone(const mrope_t *rope_in, mrope_t *rope_out) {
  ...
  if (rope_in->root_node != NULL) {
    if ((error = mrope_clone_node(rope_in->root_node, &cloned_root_node)) !=
        MROPE_OK) {
      return error;
    }
  }
...
mreturn_t mrope_delete(mrope_t *rope_inout, const size_t index,
                       const size_t length) {
  mrope_t temp_lhs, temp_rhs;
  mreturn_t error = MROPE_OK;
  ...
  if (mrope_length(rope_inout) < index ||
      mrope_length(rope_inout) < index + length) {
    return MROPE_OUT_OF_RANGE;
  }
